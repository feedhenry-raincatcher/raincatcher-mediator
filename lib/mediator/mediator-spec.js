...
describe("Mediator", function() {
  ...
  describe('topicUid', function() {
    const TEST_CHANNEL = "test:channel";
    const TEST_CHANNEL_CREATE =  "test:channel:create";

    afterEach(function() {
      mediator.remove(TEST_CHANNEL);
      mediator.remove(TEST_CHANNEL_CREATE);
    });

    it('should handle done topic UID', function(done) {
      var topicCalled = false;
      //This subscriber does not return a promise, instead publishing its own done topics
      //We have to wait for the done topic
      mediator.subscribe(TEST_CHANNEL_CREATE, function(params) {

        setTimeout(function() {
          topicCalled = true;
          var doneTopic = 'done:' + TEST_CHANNEL_CREATE + ":" + params.topicUid;
          mediator.publish(doneTopic, "test");
        }, 300);
      });

      mediator.publish(TEST_CHANNEL_CREATE, {thing: "asd", topicUid: "topicuid"}).then(function(result) {
        assert(topicCalled, "Expected the topic to be called");
        assert.equal("test", result);
        done();
      });
    });

    it('should handle error topic UID', function(done) {
      var topicCalled = false;
      //This subscriber does not return a promise, instead publishing its own done topics
      //We have to wait for the done topic
      mediator.subscribe(TEST_CHANNEL_CREATE, function(params) {
        ...
        setTimeout(function() {
          topicCalled = true;
          var doneTopic = 'error:' + TEST_CHANNEL_CREATE + ":" + params.topicUid;
          mediator.publish(doneTopic, new Error("Meh"));
        }, 300);
      });

      mediator.publish(TEST_CHANNEL_CREATE, {thing: "asd", topicUid: "topicuid"}).catch(function(err) {
        assert(topicCalled, "Expected the topic to be called");
        assert.equal("Meh", err.message);
        done();
      });
    });
    ...
    it('should handle a request', function(done) {
      ...
      mediator.subscribe(TEST_CHANNEL_CREATE, function(params) {
        ...
        setTimeout(function() {
          topicCalled = true;
          mediator.publish('done:' + TEST_CHANNEL + ":create:" + params.topicUid);
        }, 300);
      });

      mediator.request(TEST_CHANNEL_CREATE, {
        name: "thing"
      }, {uid: "topicuid"}).then(function() {
        assert(topicCalled, "Expected the topic to be called");
        done();
      });
    });

    it('should handle an error request', function(done) {
      var topicCalled = false;
      //This subscriber does not return a promise, instead publishing its own done topics
      //We have to wait for the done topic
      mediator.subscribe(TEST_CHANNEL_CREATE, function(params) {
        ...
        setTimeout(function() {
          topicCalled = true;
          mediator.publish('error:' + TEST_CHANNEL + ":create:" + params.topicUid, new Error('Meh'));
        }, 300);
      });

      mediator.request(TEST_CHANNEL_CREATE, {
        name: "thing"
      }, {uid: "topicuid"}).catch(function(err) {
        assert(topicCalled, "Expected the topic to be called");
        assert.equal("Meh", err.message);
        done();
      });
  ...
  describe("publishing", function() {
    ...
    it("should pass arguments to the given function", function() {
      ...
      sinon.assert.calledWith(spy, arg, arg2, mediator.getChannel(channel));
    });
